"""
SAM-LoRA 对比模型适配器
将 SAM + LoRA 封装为与 lssegv2.1 其他模型完全兼容的接口。

接口设计原则（零侵入）：
  - dataset.py 不需要任何改动
  - eval.py    不需要任何改动
  - train.py   只需修改 build_model 和训练/验证循环中的模型调用方式

核心适配逻辑（全部封装在本文件内）：
  1. 图像格式转换：uint8 [0-255] → SAM 标准归一化
  2. GT-derived Box Prompt：从 GT mask 动态生成边界框（与 SAM_LoRA 原项目完全一致）
...
用法（在 train.py 的 build_model 中）：
    ...
    def build_model(**params):
        model = SAMLoRA(checkpoint_path="sam_vit_b_01ec64.pth", target_size=512)
        return model

    # 训练循环中：
    preds = model(images, masks)   # SAMLoRA 接收 images + masks（用于生成 box）

    # 其他模型：
    preds = model(images)          # 普通模型只接收 images
"""
...
# ============================================================
# 以下函数来自 SAM_LoRA/model_lora.py，保持原样
# ============================================================
...
def get_lora_sam(model_type="vit_b", checkpoint_path="sam_vit_b_01ec64.pth", target_size=512, lora_r=4, lora_alpha=4):
    """构建 SAM + LoRA 模型
    
    Args:
        model_type: SAM 模型类型，默认 "vit_b"
        checkpoint_path: SAM 权重路径
        target_size: 目标图像尺寸
        lora_r: LoRA 秩（rank），控制适配器容量
        lora_alpha: LoRA 缩放因子，实际缩放 = alpha/r
    """
    print(f"Loading SAM {model_type} (Target: {target_size}, LoRA r={lora_r}, alpha={lora_alpha})...")
...
class SAMLoRA(nn.Module):
    """
    SAM-LoRA 封装模型，与 lssegv2.1 其他模型接口兼容。
    ...
    参数：
        ...
        masks  : [B, H, W] 或 [B, 1, H, W]  torch.uint8，值域 0/1
                 训练时传入 GT mask，用于生成 GT-derived Box Prompt（与 SAM_LoRA 原项目一致）
                 eval.py 中调用时不传 masks，自动退化为全图 Box Prompt

    返回：
        logits : [B, 1, H, W]  未经 sigmoid 的原始输出
    """

    # SAM 标准归一化参数（与 SAM_LoRA/dataset.py 完全一致）
    PIXEL_MEAN = torch.tensor([123.675, 116.28, 103.53]).view(1, 3, 1, 1)
    PIXEL_STD  = torch.tensor([58.395,  57.12,  57.375]).view(1, 3, 1, 1)

    def __init__(self, checkpoint_path="sam_vit_b_01ec64.pth", target_size=512, lora_r=4, lora_alpha=4):
        super().__init__()
    ...
    def _preprocess_images(self, images: torch.Tensor) -> torch.Tensor:
        """
        将 lssegv2.1 的图像格式转换为 SAM 标准格式。
        输入：uint8 [0-255]  →  输出：float，SAM 标准归一化
        """
    ...
    def _generate_boxes(self, masks: torch.Tensor, H: int, W: int) -> torch.Tensor:
        """
        从 GT mask 生成 GT-derived Box Prompt，与 SAM_LoRA/dataset.py 逻辑完全一致。
